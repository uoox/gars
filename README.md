...
- **任务模式由 SOP 驱动**：`普通 / 定时 / 触发 / 子代理 / 规划 / 目标` 六个内置模式都只是 `~/.gars/modes/builtin/*.toml` 里"选哪几个 SOP + 几个旋钮"的组合。所有任务都走同一个 `run_task` 通道。新行为 = 加 SOP + 加 mode toml，不用动 Rust 代码。
...
| `modes/{builtin,local}/` | 任务模式 toml（v0.6 新增） |
| `plans/<run_id>/plan.md` | 规划模式产物 |
| `tasks/<run_id>/<agent>/` | 子代理文件协议工作目录 |
| `runs/<run_id>/` | Goal mode / 长跑任务状态 |
...
```bash
# 1. 写一个新的 SOP（markdown，带 frontmatter）
vim ~/.gars/skills/local/my_workflow_sop.md

# 2. 写一个新的 mode TOML 引用它
cat > ~/.gars/modes/local/my_workflow.toml <<'EOF'
key = "my_workflow"
label = "我的工作流"
description = "一段话讲清楚做啥"
runner_kind = "chat"
sop_keys = ["my_workflow_sop"]
EOF

# 3. 直接新建任务即可 —— skills/ 和 modes/ 每次调用都重新扫盘
```

或者在 Web UI「模式管理」点「+ 自定义」走 REST。

> **关于重启**：gars 是基于文件扫描的，`~/.gars/skills/{local,imported}/` 和 `~/.gars/modes/{local,imported}/` 下的改动**保存即生效**，不需要重启服务。唯一需要重启的场景是升级 gars 二进制本身（启动时会从内嵌资源刷新 `builtin/` 子目录，对 `local/` 永不动）。

---

## REST API（速查）

`/health` 公开；其它 `/v1/*` 都需要 `Authorization: Bearer <admin_token>`。
...
GET  /v1/modes            POST /v1/modes
GET  /v1/modes/{key}      DELETE /v1/modes/{key}
